...
const ForgotPassword = () => {
  ...
  const sendOtp = async () => {
    try {
      ...
      // Backend returns the 4-digit OTP as res.data.data string
      const receivedOtp = res.data?.data;
      if (receivedOtp && /^\d{4}$/.test(String(receivedOtp))) {
        setDemoOtp(String(receivedOtp));
      } else {
        setDemoOtp('');
      }
    } catch (err) {
      ...
      {showOtpPopup && (
        <div style={{
          ...
          <div style={{
            ...
            {demoOtp ? (
              <div style={{
                background: '#f0fff4', border: '2px solid #76C442', borderRadius: '10px',
                padding: '12px 16px', marginBottom: '16px', textAlign: 'center',
              }}>
                <p style={{ fontSize: '12px', color: '#276749', margin: '0 0 4px', fontWeight: 600 }}>Your OTP is</p>
                <p style={{ fontSize: '28px', fontWeight: 800, color: '#276749', letterSpacing: '8px', margin: 0 }}>{demoOtp}</p>
              </div>
            ) : (
              <div style={{
                background: '#fffaf0', border: '1px solid #fbd38d', borderRadius: '8px',
                ...
                OTP sent — check server console logs for the code
              </div>
            )}
