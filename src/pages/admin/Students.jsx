...
export default function Students() {
  ...
  const handleDelete = async (id) => {
    const ok = await apiDeleteStudent(id);
    setDeleteId(null);
    if (ok) {
      ...
      <div className="data-table-card">
        <div className="search-filter-bar">
          ...
          <select className="filter-select" value={filterClass} onChange={e => { setFilterClass(e.target.value); setCurrentPage(1); }}>
            ...
            {CLASS_FILTER_OPTIONS.map(group => (
              <optgroup key={group.group} label={group.group}>
                {group.items.map(val => (
                  <option key={val} value={val}>{['Nursery','LKG','UKG'].includes(val) ? val : `Class ${val}`}</option>
                ))}
              </optgroup>
            ))}
          </select>
            ...
            <tbody>
              {loadingStudents ? (
                ...
                <tr key={s.id}>
                  ...
                  <td><span style={{ fontSize: '13px', fontWeight: 700 }}>Class {s.class}{s.section ? `-${s.section}` : ''}</span></td>
                ...
                <div className="modal-body" style={{ maxHeight: '72vh', overflowY: 'auto', padding: '20px 24px' }}>
                  ...
                  <div className="row g-3">
                    ...
                    <div className="col-md-3">
                      ...
                      <input type="text" className={`form-control form-control-sm ${errors.class ? 'is-invalid' : ''}`}
                        placeholder="e.g., 10" value={formData.class}
                        onChange={set('class')} />
                    ...
                    <div className="col-md-3">
                      ...
                      <input type="text" className="form-control form-control-sm"
                        placeholder="e.g., A" value={formData.section}
          ...
          <div className="modal-dialog modal-sm">
            <div className="modal-content">
              <div className="modal-body text-center p-4">
                ...
                <h5 className="mt-2">Delete Student?</h5>
