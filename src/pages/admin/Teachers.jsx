              ...
              {filtered.length === 0 ? (
                ...
                return (
                  <tr key={t.id}>
                    ...
                    <td style={{ fontSize: 12, color: '#718096', maxWidth: 120, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{t.classes || '—'}</td>
          ...
          <div className="modal-container" style={{ maxWidth: 640 }}>
            ...
            <form onSubmit={handleSave}>
              <div className="modal-body" style={{ maxHeight: '72vh', overflowY: 'auto', padding: '20px 24px' }}>
                ...
                {/* ── Classes ──────────────────────────────────────────── */}
                <Section icon="class" label="Assigned Classes" />
                <div>
                  <label style={labelStyle}>Classes <span style={{ fontWeight: 400, color: '#a0aec0', fontSize: 11 }}>(Optional — comma-separated, e.g. Nursery-A, 1-B, 10-A)</span></label>
                  <input style={inputStyle(false)} placeholder="e.g., Nursery-A, LKG-B, 1-A, 5-B, 10-A" value={form.classes}
                    onChange={e => setForm({ ...form, classes: e.target.value })} />
                  <p style={{ margin: '5px 0 0', fontSize: 11, color: '#a0aec0' }}>
                    Enter class names separated by commas. Sections range from Nursery / LKG / UKG to Class 12 (e.g., 1-A, 2-B, 12-C).
                  </p>
                </div>
