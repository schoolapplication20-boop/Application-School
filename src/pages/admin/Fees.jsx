...
const EMPTY_FORM = { student: '', rollNo: '', class: '', type: 'Tuition Fee', amount: '', month: '', dueDate: '', status: 'PENDING', paymentMethod: '', paidDate: '' };
...
export default function Fees() {
  ...
  useEffect(() => {
    apiFetchFees().then(data => { if (data && data.length > 0) setFees(data); });
  }, []);
  ...
  const confirmPaid = async (method, date) => {
    const updates = { status: 'PAID', paidDate: date, paidAmount: payTarget.amount, paymentMethod: method };
    await apiUpdateFee(payTarget.id, updates);
    const updated = fees.map(f => f.id === payTarget.id ? { ...f, ...updates } : f);
    persist(updated);
    setPayTarget(null);
  ...
  const handleAddFee = async (e) => {
    e.preventDefault();
    if (!formData.student.trim() || !formData.amount) { showToast('Student name and amount are required', 'warning'); return; }
    ...
    const created = await apiCreateFee(payload);
    persist([...fees, created]);
    setShowModal(false);
                ...
                <div className="modal-body">
                  <div className="row g-3">
                    <div className="col-md-6">
                      ...
                      <input type="text" className="form-control form-control-sm" placeholder="Student name"
                        value={formData.student} onChange={e => setFormData({...formData, student: e.target.value})} />
                    </div>
                    <div className="col-md-3">
                      <label className="form-label small fw-medium">Roll No</label>
                      <input type="text" className="form-control form-control-sm" placeholder="e.g., S001"
                    ...
                    <div className="col-md-3">
                      ...
                      <input type="text" className="form-control form-control-sm" placeholder="e.g., 10-A"
