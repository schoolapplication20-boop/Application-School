  ...
  const handleSave = async () => {
    ...
    try {
      if (editAdmin) {
        ...
        const result = res.data?.data; // { id, name, email, mobile, generatedPassword }
        addLog('Super Admin', `Created new admin: ${formData.name}`, 'Admin Management');
        setShowModal(false);
        setFormData(emptyForm);
        setFormErrors({});
        setShowPassword(false);
        setSuccessModal({ name: result.name, email: result.email, password: result.generatedPassword });
        loadAdmins();
      }
    } catch (err) {
        ...
        <div className="modal-overlay">
          <div className="modal-container" style={{ maxWidth: 460 }}>
            ...
            <div className="modal-body" style={{ padding: '24px' }}>
              <p style={{ fontSize: '13px', color: '#4a5568', marginBottom: '20px' }}>
                The admin account has been created. Share the credentials below with <strong>{successModal.name}</strong>.
              </p>

              <div style={{ background: '#f7fafc', border: '1px solid #e2e8f0', borderRadius: '10px', padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <div>
                  <div style={{ fontSize: '11px', fontWeight: 700, color: '#a0aec0', textTransform: 'uppercase', marginBottom: '4px' }}>Name</div>
                  <div style={{ fontSize: '14px', fontWeight: 600, color: '#2d3748' }}>{successModal.name}</div>
                </div>
                <div>
                  <div style={{ fontSize: '11px', fontWeight: 700, color: '#a0aec0', textTransform: 'uppercase', marginBottom: '4px' }}>Email (Login ID)</div>
                  <div style={{ fontSize: '14px', fontWeight: 600, color: '#2d3748' }}>{successModal.email}</div>
                </div>
                <div>
                  <div style={{ fontSize: '11px', fontWeight: 700, color: '#a0aec0', textTransform: 'uppercase', marginBottom: '4px' }}>Generated Password</div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <div style={{ flex: 1, fontSize: '15px', fontWeight: 700, color: '#276749', fontFamily: 'monospace', letterSpacing: '0.1em', background: '#f0fff4', border: '1px solid #c6f6d5', borderRadius: '8px', padding: '8px 12px' }}>
                      {showPassword ? successModal.password : '•'.repeat(successModal.password.length)}
                    </div>
                    <button onClick={() => setShowPassword(v => !v)} title={showPassword ? 'Hide' : 'Show'}
                      style={{ padding: '8px', border: '1px solid #e2e8f0', borderRadius: '8px', background: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center' }}>
                      <span className="material-icons" style={{ fontSize: '18px', color: '#718096' }}>
                        {showPassword ? 'visibility_off' : 'visibility'}
                      </span>
                    </button>
                    <button onClick={() => { navigator.clipboard.writeText(successModal.password); showToast('Password copied!'); }} title="Copy"
                      style={{ padding: '8px', border: '1px solid #e2e8f0', borderRadius: '8px', background: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center' }}>
                      <span className="material-icons" style={{ fontSize: '18px', color: '#718096' }}>content_copy</span>
                    </button>
                  </div>
                </div>
              </div>

              <div style={{ marginTop: '16px', padding: '10px 12px', background: '#fff5f5', border: '1px solid #fed7d7', borderRadius: '8px', fontSize: '12px', color: '#c53030', display: 'flex', gap: '6px', alignItems: 'flex-start' }}>
                <span className="material-icons" style={{ fontSize: '16px', marginTop: '1px', flexShrink: 0 }}>warning</span>
                <span>This password will not be shown again. Make sure to copy or note it before closing this dialog.</span>
              </div>
            </div>
            <div className="modal-footer">
              <button onClick={() => { setSuccessModal(null); setShowPassword(false); }}
                style={{ padding: '10px 28px', background: '#76C442', border: 'none', borderRadius: '8px', color: '#fff', fontWeight: 700, cursor: 'pointer' }}>
                Done
              </button>
            </div>
          </div>
