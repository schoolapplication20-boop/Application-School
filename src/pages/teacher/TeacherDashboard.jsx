...
export default function TeacherDashboard() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetchStudents(),
      fetchTimetable(),
    ]).then(([allTeachers, allStudents, allEntries]) => {
      // 1. Find teacher profile: match by userId (auth record id) or email
      const profile =
      ...
      // 2. Parse assigned classes
      const classes = profile ? parseClasses(profile.classes) : [];
      setAssignedClasses(classes);

      // 3. Filter students in those classes
      if (classes.length > 0) {
  ...
  const studentsByClass = useMemo(() => {
    const map = {};
    assignedClasses.forEach(cls => { map[cls] = []; });
        ...
        {assignedClasses.length === 0 ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 16, padding: 20 }}>
            {assignedClasses.map((cls) => {
              ...
              return (
                <div key={cls} style={{ border: `1.5px solid ${color}30`, borderRadius: 12, padding: '16px 18px', background: color + '08' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                    <div>
                      <div style={{ fontSize: 18, fontWeight: 800, color, lineHeight: 1 }}>Class</div>
                      <div style={{ fontSize: 26, fontWeight: 800, color: '#1a202c', lineHeight: 1.1 }}>{cls}</div>
                    </div>
