...
import { categoryLabel, APPROVAL_STATUS_META } from './constants';
  ...
  const handleSave = async (data) => {
    ...
    try {
      const res = modalTemplate
        ? await whatsappAPI.updateTemplate(modalTemplate.id, data)
  ...
  return (
    <div className="data-table-card">
      <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ fontWeight: 700, fontSize: 14, color: 'var(--text-primary)' }}>WhatsApp Templates</span>
        <button onClick={() => setModalTemplate(null)}
          style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#25D366', color: '#fff', border: 'none', borderRadius: 8, padding: '8px 16px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
      ...
      {loading ? (
        ...
        <div style={{ padding: 40, textAlign: 'center', color: 'var(--text-muted)', fontSize: 13 }}>
          ...
          No templates yet. Add one that matches a message template already approved in your Meta Business Manager.
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          {templates.map(t => {
            const approval = APPROVAL_STATUS_META[t.approvalStatus] || {};
            return (
              <div key={t.id} style={{ padding: '14px 20px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4, flexWrap: 'wrap' }}>
                    ...
                    <span style={{ padding: '2px 8px', borderRadius: 10, fontSize: 10, fontWeight: 700, background: approval.bg, color: approval.color }}>{approval.label || t.approvalStatus}</span>
                    {!t.isActive && (
                  ...
                  <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 4 }}>
                    Meta template: <code>{t.metaTemplateName}</code> ({t.metaLanguageCode})
                  </div>
                  {t.contentPreview && (
                    <div style={{ fontSize: 13, color: 'var(--text-secondary)', whiteSpace: 'pre-wrap' }}>{t.contentPreview}</div>
                  )}
                </div>
