...
import { LOG_STATUS_META, categoryLabel, formatDateTime } from './constants';
...
export default function HistoryTab({ showToast }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await whatsappAPI.getHistory(params);
      const data = res.data?.data;
      setLogs(data?.content ?? []);
      setTotalPages(data?.totalPages ?? 0);
      setTotalElements(data?.totalElements ?? 0);
    } catch (e) {
      showToast(e.response?.data?.message || 'Failed to load WhatsApp history', 'error');
      setLogs([]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="data-table-card">
      <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border)', display: 'flex', flexWrap: 'wrap', gap: 10, alignItems: 'center' }}>
        <span style={{ fontWeight: 700, fontSize: 14, color: 'var(--text-primary)', marginRight: 8 }}>WhatsApp History</span>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ background: 'var(--surface-alt)', textAlign: 'left' }}>
                ...
                <th style={{ padding: '10px 16px', fontWeight: 700, color: 'var(--text-secondary)' }}>Category</th>
              ...
              {logs.map(log => {
                ...
                return (
                  <tr key={log.id} style={{ borderBottom: '1px solid var(--border)' }}>
                    ...
                    <td style={{ padding: '10px 16px', color: 'var(--text-secondary)' }}>{categoryLabel(log.category)}</td>
                    <td style={{ padding: '10px 16px', maxWidth: 360 }}>
                      <div style={{ color: 'var(--text-secondary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{log.renderedPreview || '—'}</div>
