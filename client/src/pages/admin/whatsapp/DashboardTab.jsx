    ...
    <div>
      ...
      <div style={{
        ...
        {stats.providerConfigured
          ...
          : 'WhatsApp is not configured yet — go to Settings to connect your Meta phone number.'}
      </div>
      ...
      {stats.quota != null && (
        <div style={{
          ...
          background: stats.quotaBreached ? '#fff5f5' : 'var(--surface-alt)',
          color: stats.quotaBreached ? '#c53030' : 'var(--text-secondary)',
          fontSize: 13, fontWeight: 600,
        }}>
          <span className="material-icons" style={{ fontSize: 18 }}>{stats.quotaBreached ? 'error_outline' : 'speed'}</span>
          {stats.sentThisMonth} / {stats.quota} messages used this month
          {stats.quotaBreached ? ' — quota exceeded (sending is not blocked, but check with My-Skoolz support about your plan).' : ''}
        </div>
      )}
        ...
        <div className="data-table-card">
          ...
          {(!stats.recentCampaigns || stats.recentCampaigns.length === 0) ? (
            <div style={{ padding: 40, textAlign: 'center', color: 'var(--text-muted)', fontSize: 13 }}>
              <span className="material-icons" style={{ fontSize: 40, display: 'block', marginBottom: 8, color: 'var(--border-strong)' }}>chat</span>
              No campaigns yet.
            </div>
          ) : (
