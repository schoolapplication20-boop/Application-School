      ...
      <div className="modal-overlay">
        <div className="modal-container" style={{ maxWidth: 420 }}>
          <div className="modal-body" style={{ textAlign: 'center', padding: '32px 28px' }}>
            ...
            <h3 style={{ fontSize: 17, fontWeight: 700, margin: '0 0 12px', color: '#c53030' }}>
              Delete {count} Student{count !== 1 ? 's' : ''}?
            </h3>
            <p style={{ fontSize: 13, color: 'var(--text-secondary)', margin: '0 0 20px', lineHeight: 1.6 }}>
              Are you sure you want to permanently delete{' '}
              <strong style={{ color: 'var(--text-primary)' }}>{count} student{count !== 1 ? 's' : ''}</strong>?
            ...
            <div style={{ padding: '10px 14px', background: '#fff5f5', border: '1px solid #fed7d7', borderRadius: 8, fontSize: 12, color: '#c53030', marginBottom: 24, textAlign: 'left', display: 'flex', gap: 8 }}>
              ...
              <span>All selected student records and linked login accounts will be permanently removed.</span>
            </div>
            <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
              ...
              <button
                onClick={() => onConfirm()}
                disabled={bulkDeleting}
                style={{ padding: '9px 22px', background: bulkDeleting ? '#a0aec0' : '#e53e3e', color: '#fff', border: 'none', borderRadius: 9, fontWeight: 700, fontSize: 13, cursor: bulkDeleting ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
                {bulkDeleting
      ...
      <div className="modal-container" style={{ maxWidth: 440 }}>
        <div className="modal-body" style={{ textAlign: 'center', padding: '32px 28px' }}>
          ...
          <h3 style={{ fontSize: 17, fontWeight: 700, margin: '0 0 8px', color: 'var(--text-primary)' }}>
            Request Deletion of {count} Student{count !== 1 ? 's' : ''}?
          </h3>
          <p style={{ fontSize: 13, color: 'var(--text-secondary)', margin: '0 0 16px' }}>
            You have selected <strong style={{ color: 'var(--text-primary)' }}>{count}</strong> student{count !== 1 ? 's' : ''} for deletion.
          </p>
          <div style={{ background: '#fff7ed', border: '1px solid #fed7aa', borderRadius: 8, padding: '10px 14px', fontSize: 12, color: '#9a3412', marginBottom: 16, textAlign: 'left', display: 'flex', gap: 8 }}>
            ...
            <span>Each student will get a deletion request sent to the Super Admin for approval — none will be removed until approved.</span>
          </div>
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
            ...
            <button
              ...
              {bulkDeleting
                ...
                : `Submit ${count} Student${count !== 1 ? 's' : ''} for Approval`}
            </button>
          </div>
        </div>
      </div>
