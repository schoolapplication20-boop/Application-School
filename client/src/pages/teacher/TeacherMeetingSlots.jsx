...
export default function TeacherMeetingSlots() {
  ...
  const inp = { width: '100%', padding: '9px 12px', border: '1px solid var(--border-strong)', borderRadius: 8, fontSize: 13, outline: 'none', boxSizing: 'border-box' };
    ...
    <Layout pageTitle="Meeting Slots">
      <div style={{ maxWidth: 900, margin: '0 auto', padding: 24 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
          <div>
            <h1 style={{ fontSize: 24, fontWeight: 700, color: 'var(--text-primary)', margin: 0 }}>Parent-Teacher Meeting Slots</h1>
            <p style={{ color: 'var(--text-secondary)', marginTop: 4, fontSize: 14 }}>Create available slots for parents to book meetings</p>
          </div>
        ...
        {/* Create Form */}
        {showForm && (
          <div style={{ background: 'var(--surface)', borderRadius: 12, boxShadow: '0 2px 8px rgba(0,0,0,0.1)', padding: 24, marginBottom: 24 }}>
            ...
            <form onSubmit={handleCreate}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14, marginBottom: 14 }}>
                ...
                <div>
                  <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-secondary)', display: 'block', marginBottom: 5 }}>Start Time *</label>
                  <input type="time" value={form.startTime} onChange={e => setForm(f => ({ ...f, startTime: e.target.value }))} style={inp} />
                </div>
                <div>
                  <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-secondary)', display: 'block', marginBottom: 5 }}>End Time *</label>
                  <input type="time" value={form.endTime} onChange={e => setForm(f => ({ ...f, endTime: e.target.value }))} style={inp} />
                </div>
                <div style={{ gridColumn: '1/-1' }}>
                  <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-secondary)', display: 'block', marginBottom: 5 }}>Topic / Agenda</label>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                ...
                <button type="button" onClick={() => setShowForm(false)} style={{ padding: '9px 22px', background: 'var(--surface-alt)', color: 'var(--text-secondary)', border: 'none', borderRadius: 8, fontSize: 13, cursor: 'pointer' }}>Cancel</button>
              </div>
            </form>
          </div>
        )}
        ...
        {loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {slots.map(({ slot, bookings }) => (
              <div key={slot.id} style={{ background: 'var(--surface)', borderRadius: 12, boxShadow: '0 1px 4px rgba(0,0,0,0.08)', overflow: 'hidden' }}>
                <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                    ...
                    <div>
                      <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--text-primary)' }}>{fmt(slot.meetingDate)}</div>
                      <div style={{ fontSize: 13, color: 'var(--text-secondary)' }}>{slot.startTime} – {slot.endTime}</div>
                      {slot.topic && <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 2 }}>📋 {slot.topic}</div>}
                    </div>
                  </div>
                ...
                {bookings && bookings.length > 0 && (
                  <div style={{ padding: '12px 20px' }}>
                    <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-secondary)', marginBottom: 8 }}>BOOKINGS</div>
                    {bookings.map(b => (
                      <div key={b.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: '1px solid var(--border)' }}>
                        <span className="material-icons" style={{ fontSize: 18, color: String(b.status || '').toUpperCase() === 'CONFIRMED' ? '#48bb78' : 'var(--text-muted)' }}>
                          {String(b.status || '').toUpperCase() === 'CONFIRMED' ? 'check_circle' : 'cancel'}
                        </span>
                        <div style={{ flex: 1 }}>
                          <div style={{ fontWeight: 600, fontSize: 13 }}>{b.studentName}</div>
                          {b.parentName && <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>Parent: {b.parentName}</div>}
                          {b.parentEmail && <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>Email: {b.parentEmail}</div>}
                          {b.notes && <div style={{ fontSize: 12, color: 'var(--text-secondary)', fontStyle: 'italic' }}>Note: {b.notes}</div>}
                        </div>
