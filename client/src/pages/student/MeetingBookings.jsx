    ...
    <Layout pageTitle="Meeting Bookings">
      <div style={{ maxWidth: 860, margin: '0 auto', padding: 24 }}>
        ...
        <div style={{ marginBottom: 24 }}>
          <h1 style={{ fontSize: 24, fontWeight: 700, color: 'var(--text-primary)', margin: 0 }}>Parent-Teacher Meetings</h1>
          <p style={{ color: 'var(--text-secondary)', marginTop: 4, fontSize: 14 }}>Book a meeting slot with your teacher</p>
        </div>
        ...
        {/* Tabs */}
        <div style={{ display: 'flex', gap: 8, marginBottom: 20, borderBottom: '2px solid var(--border-strong)', paddingBottom: 0 }}>
        ...
        {loading ? (
          ...
          slots.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {slots.map(slot => (
                <div key={slot.id} style={{ background: 'var(--surface)', borderRadius: 12, boxShadow: '0 1px 4px rgba(0,0,0,0.08)', padding: 20, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 14 }}>
                  <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
                    ...
                    <div>
                      <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--text-primary)' }}>{slot.teacherName}</div>
                      <div style={{ fontSize: 13, color: 'var(--text-secondary)' }}>{fmt(slot.meetingDate)} · {slot.startTime} – {slot.endTime}</div>
                      {slot.topic && <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 4 }}>📋 {slot.topic}</div>}
                    </div>
                  </div>
                  <div>
                    {booking === slot.id ? (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, minWidth: 220 }}>
                        <input value={noteInput} onChange={e => setNoteInput(e.target.value)}
                          ...
                          style={{ padding: '8px 12px', border: '1px solid var(--border-strong)', borderRadius: 8, fontSize: 13, outline: 'none' }} />
                        <div style={{ display: 'flex', gap: 8 }}>
                          ...
                          <button onClick={() => setBooking(null)}
                            style={{ padding: '8px 14px', background: 'var(--surface-alt)', color: 'var(--text-secondary)', border: 'none', borderRadius: 8, fontSize: 13, cursor: 'pointer' }}>
                            Cancel
                          </button>
                        </div>
                      </div>
          ...
          bookings.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {bookings.map(({ booking: b, slot }) => (
                <div key={b.id} style={{ background: 'var(--surface)', borderRadius: 12, boxShadow: '0 1px 4px rgba(0,0,0,0.08)', padding: 20, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
                  <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
                    <span className="material-icons" style={{ fontSize: 28, color: String(b.status || '').toUpperCase() === 'CONFIRMED' ? '#48bb78' : 'var(--text-muted)', marginTop: 2 }}>
                      {String(b.status || '').toUpperCase() === 'CONFIRMED' ? 'check_circle' : 'cancel'}
                    </span>
                    <div>
                      {slot && <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--text-primary)' }}>{slot.teacherName}</div>}
                      {slot && <div style={{ fontSize: 13, color: 'var(--text-secondary)' }}>{fmt(slot.meetingDate)} · {slot.startTime} – {slot.endTime}</div>}
                      {slot?.topic && <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 3 }}>📋 {slot.topic}</div>}
                      {b.notes && <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 3, fontStyle: 'italic' }}>Note: {b.notes}</div>}
                    </div>
                  </div>
