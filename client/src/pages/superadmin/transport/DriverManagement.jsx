    ...
    <div style={{ padding: 28, background: 'var(--surface-alt)', minHeight: '100vh' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            <h1 style={{ margin: 0, fontSize: 22, fontWeight: 800, color: 'var(--text-primary)' }}>Driver Management</h1>
            <div style={{ fontSize: 13, color: 'var(--text-secondary)' }}>Manage bus drivers, licenses, and assignments</div>
          </div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(160px,1fr))', gap: 16, marginBottom: 24 }}>
        {[
          ...
          { label: 'Inactive', value: inactiveCount, color: 'var(--text-secondary)', bg: 'var(--surface-alt)', icon: 'cancel' },
          { label: 'Assigned to Bus', value: drivers.filter(d => d.busId).length, color: '#7c3aed', bg: '#faf5ff', icon: 'directions_bus' },
        ].map(({ label, value, color, bg, icon }) => (
          <div key={label} style={{ background: 'var(--surface)', borderRadius: 14, padding: '18px 20px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', display: 'flex', alignItems: 'center', gap: 14 }}>
      ...
      {/* Filters */}
      <div style={{ background: 'var(--surface)', borderRadius: 14, padding: '16px 20px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
        <div style={{ position: 'relative', flex: 1, minWidth: 200 }}>
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search drivers..."
            style={{ width: '100%', paddingLeft: 36, paddingRight: 12, paddingTop: 9, paddingBottom: 9, border: '1px solid var(--border-strong)', borderRadius: 8, fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
        </div>
          ...
          {['All', 'Active', 'Inactive', 'On Leave'].map(s => (
            <button key={s} onClick={() => setStatusFilter(s)}
              style={{ padding: '7px 16px', borderRadius: 8, border: '1px solid', fontSize: 13, fontWeight: 600, cursor: 'pointer',
                background: statusFilter === s ? '#76C442' : 'var(--surface)',
                color: statusFilter === s ? '#fff' : 'var(--text-secondary)',
                borderColor: statusFilter === s ? '#76C442' : 'var(--border-strong)' }}>
              {s}
            </button>
          ))}
        </div>
      ...
      {/* Table */}
      <div style={{ background: 'var(--surface)', borderRadius: 14, boxShadow: '0 2px 8px rgba(0,0,0,0.06)', overflow: 'hidden' }}>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ background: 'var(--surface-alt)' }}>
                {['Driver', 'Employee ID', 'License', 'Expiry', 'Phone', 'Bus Assigned', 'Experience', 'Status', 'Actions'].map(h => (
                  <th key={h} style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 700, color: 'var(--text-secondary)', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.05em', whiteSpace: 'nowrap' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.length === 0 ? (
                ...
                return (
                  <tr key={d.id} style={{ borderTop: '1px solid var(--border)' }}>
                    ...
                    <td style={{ padding: '12px 16px', color: 'var(--text-secondary)' }}>{d.employeeId || '—'}</td>
                    <td style={{ padding: '12px 16px', color: 'var(--text-secondary)', fontFamily: 'monospace' }}>{d.licenseNo || '—'}</td>
                    <td style={{ padding: '12px 16px', color: d.licenseExpiry && new Date(d.licenseExpiry) < new Date() ? '#e53e3e' : 'var(--text-secondary)' }}>{d.licenseExpiry || '—'}</td>
                    ...
                    <td style={{ padding: '12px 16px', color: 'var(--text-secondary)' }}>{d.experience ? `${d.experience} yrs` : '—'}</td>
                    <td style={{ padding: '12px 16px' }}>
      ...
      {showModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 16 }}>
          <div className="modal-card" style={{ background: 'var(--surface)', borderRadius: 16, width: '100%', maxWidth: 640, maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
            <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <h2 style={{ margin: 0, fontSize: 17, fontWeight: 800, color: 'var(--text-primary)' }}>{editDriver ? 'Edit Driver' : 'Add New Driver'}</h2>
            ...
            <form onSubmit={handleSave} style={{ padding: 24 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                {[
                  ...
                  <div key={key} style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    ...
                    <input type={type} value={form[key]} onChange={e => setForm({ ...form, [key]: e.target.value })}
                      ...
                      style={{ padding: '9px 12px', border: '1px solid var(--border-strong)', borderRadius: 8, fontSize: 13, outline: 'none' }} />
                  </div>
                ))}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-secondary)' }}>Assign Bus</label>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 16 }}>
                ...
                <textarea value={form.address} onChange={e => setForm({ ...form, address: e.target.value })}
                  ...
                  style={{ padding: '9px 12px', border: '1px solid var(--border-strong)', borderRadius: 8, fontSize: 13, outline: 'none', resize: 'vertical' }} />
              </div>
              <div style={{ display: 'flex', gap: 12, marginTop: 24, justifyContent: 'flex-end' }}>
                <button type="button" onClick={() => setShowModal(false)}
                  style={{ padding: '10px 24px', borderRadius: 8, border: '1px solid var(--border-strong)', background: 'var(--surface)', color: 'var(--text-secondary)', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>
                  Cancel
                </button>
      ...
      {deleteTarget && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 16 }}>
          <div className="modal-card" style={{ background: 'var(--surface)', borderRadius: 16, padding: 28, maxWidth: 400, width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.2)', textAlign: 'center' }}>
            ...
            <h3 style={{ margin: '0 0 8px', fontSize: 18, fontWeight: 800, color: 'var(--text-primary)' }}>Delete Driver?</h3>
            <p style={{ margin: '0 0 24px', color: 'var(--text-secondary)', fontSize: 14 }}>This will permanently delete <strong>{deleteTarget.name}</strong>. This action cannot be undone.</p>
            <div style={{ display: 'flex', gap: 12 }}>
              <button onClick={() => setDeleteTarget(null)} style={{ flex: 1, padding: '10px', borderRadius: 8, border: '1px solid var(--border-strong)', background: 'var(--surface)', color: 'var(--text-secondary)', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>Cancel</button>
