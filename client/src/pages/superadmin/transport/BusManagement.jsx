...
function Modal({ open, title, children, onClose, onSave, saving }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div className="modal-card" style={{ background: 'var(--surface)', borderRadius: 16, width: '100%', maxWidth: 580, maxHeight: '90vh', overflow: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
        <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <h2 style={{ margin: 0, fontSize: 17, fontWeight: 700, color: 'var(--text-primary)' }}>{title}</h2>
          <button onClick={onClose} style={{ border: 'none', background: 'none', fontSize: 22, cursor: 'pointer', color: 'var(--text-muted)' }}>×</button>
        </div>
        <div style={{ padding: '20px 24px' }}>{children}</div>
        <div style={{ padding: '16px 24px', borderTop: '1px solid var(--border)', display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
          <button onClick={onClose} style={{ padding: '9px 20px', border: '1.5px solid var(--border-strong)', borderRadius: 8, background: 'var(--surface)', fontWeight: 600, cursor: 'pointer', fontSize: 13, color: 'var(--text-secondary)' }}>Cancel</button>
...
const Field = ({ label, children, required }) => (
  <div style={{ marginBottom: 14 }}>
    <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: 'var(--text-secondary)', marginBottom: 5 }}>{label}{required && <span style={{ color: '#e53e3e' }}> *</span>}</label>
    {children}
  </div>
);

const inp = { width: '100%', padding: '9px 12px', border: '1.5px solid var(--border-strong)', borderRadius: 8, fontSize: 13, color: 'var(--text-primary)', outline: 'none', boxSizing: 'border-box', background: 'var(--surface)' };
...
export default function BusManagement() {
  ...
  return (
    <div style={{ padding: 28, background: 'var(--surface-alt)', minHeight: '100vh' }}>
      {/* Header */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        <div>
          <h1 style={{ margin: 0, fontSize: 20, fontWeight: 800, color: 'var(--text-primary)' }}>Bus Management</h1>
          <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginTop: 2 }}>{buses.length} buses registered</div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px,1fr))', gap: 14, marginBottom: 22 }}>
        {[
          ...
          <div key={label} style={{ background: 'var(--surface)', borderRadius: 14, padding: '16px 18px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)', display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              <div style={{ fontSize: 22, fontWeight: 800, color: 'var(--text-primary)' }}>{val}</div>
      ...
      {/* Filters */}
      <div style={{ background: 'var(--surface)', borderRadius: 14, padding: '14px 18px', marginBottom: 18, boxShadow: '0 2px 8px rgba(0,0,0,0.05)', display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        {['All', 'Active', 'Inactive', 'Maintenance'].map(s => (
          <button key={s} onClick={() => setStatusFilter(s)}
            style={{ padding: '7px 16px', borderRadius: 8, border: '1.5px solid', borderColor: statusFilter === s ? '#76C442' : 'var(--border-strong)', background: statusFilter === s ? '#f0fff4' : 'var(--surface)', color: statusFilter === s ? '#276749' : 'var(--text-secondary)', fontWeight: 600, fontSize: 13, cursor: 'pointer' }}>
            {s}
          </button>
        ))}
      </div>
      ...
      {/* Table */}
      <div style={{ background: 'var(--surface)', borderRadius: 14, boxShadow: '0 2px 8px rgba(0,0,0,0.05)', overflow: 'auto' }}>
        {loading ? (
          <div style={{ textAlign: 'center', padding: 48, color: 'var(--text-muted)' }}>
            <div style={{ width: 32, height: 32, border: '3px solid var(--border-strong)', borderTopColor: '#76C442', borderRadius: '50%', animation: 'spin 0.8s linear infinite', margin: '0 auto 12px' }} />
            Loading buses…
          </div>
        ) : filtered.length === 0 ? (
          <div style={{ textAlign: 'center', padding: 48, color: 'var(--text-muted)' }}>
            ...
            <thead>
              <tr style={{ background: 'var(--surface-alt)' }}>
                {['Bus No', 'Registration', 'Model', 'Driver', 'Route', 'Capacity', 'GPS Device', 'Status', 'Actions'].map(h => (
                  <th key={h} style={{ padding: '12px 14px', textAlign: 'left', fontWeight: 700, color: 'var(--text-secondary)', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.05em', whiteSpace: 'nowrap' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.map(b => {
                ...
                return (
                  <tr key={b.id} style={{ borderTop: '1px solid var(--border)' }}>
                    ...
                    <td style={{ padding: '12px 14px', color: 'var(--text-secondary)' }}>{b.registrationNo || '—'}</td>
                    <td style={{ padding: '12px 14px', color: 'var(--text-secondary)' }}>{b.model || '—'}</td>
                    <td style={{ padding: '12px 14px', color: 'var(--text-secondary)' }}>{b.driver || '—'}</td>
                    <td style={{ padding: '12px 14px', color: 'var(--text-secondary)' }}>{b.route || '—'}</td>
                    <td style={{ padding: '12px 14px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        <span style={{ fontWeight: 600, color: 'var(--text-primary)' }}>{b.currentStudents || 0}/{b.capacity}</span>
                        <div style={{ height: 5, width: 50, background: 'var(--border-strong)', borderRadius: 5, overflow: 'hidden' }}>
                          <div style={{ height: '100%', width: `${b.capacity > 0 ? Math.round(((b.currentStudents || 0) / b.capacity) * 100) : 0}%`, background: '#76C442', borderRadius: 5 }} />
                        </div>
                      </div>
                    </td>
                    <td style={{ padding: '12px 14px', color: b.gpsDeviceId ? '#276749' : 'var(--text-muted)', fontSize: 12 }}>
                      {b.gpsDeviceId ? (
      ...
      {deleteId && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 1001, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div className="modal-card" style={{ background: 'var(--surface)', borderRadius: 16, padding: 28, maxWidth: 380, width: '100%', textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
            <span className="material-icons" style={{ fontSize: 44, color: '#e53e3e', display: 'block', marginBottom: 12 }}>delete_forever</span>
            <h3 style={{ margin: '0 0 8px', color: 'var(--text-primary)' }}>Delete Bus?</h3>
            <p style={{ margin: '0 0 22px', color: 'var(--text-secondary)', fontSize: 14 }}>This action cannot be undone. All assignment data for this bus will be affected.</p>
            <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
              <button onClick={() => setDeleteId(null)} style={{ padding: '9px 22px', border: '1.5px solid var(--border-strong)', borderRadius: 8, background: 'var(--surface)', fontWeight: 600, cursor: 'pointer' }}>Cancel</button>
