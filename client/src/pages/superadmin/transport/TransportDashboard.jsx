...
const StatCard = ({ icon, label, value, sub, color, bg }) => (
  <div style={{ background: '#fff', borderRadius: 16, padding: '22px 24px', boxShadow: '0 2px 12px rgba(0,0,0,0.06)', display: 'flex', alignItems: 'center', gap: 18 }}>
    ...
    <div>
      <div style={{ fontSize: 26, fontWeight: 800, color: '#1a202c', lineHeight: 1 }}>{value}</div>
      <div style={{ fontSize: 13, fontWeight: 600, color: '#4a5568', marginTop: 3 }}>{label}</div>
...
const QuickLink = ({ to, icon, label, color }) => (
  <Link to={to} style={{ textDecoration: 'none' }}>
    <div style={{ background: '#fff', borderRadius: 14, padding: '18px 20px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', display: 'flex', alignItems: 'center', gap: 14, cursor: 'pointer', transition: 'box-shadow 0.2s' }}
      ...
      <span className="material-icons" style={{ fontSize: 18, color: '#cbd5e0', marginLeft: 'auto' }}>chevron_right</span>
    </div>
  </Link>
);
    ...
    <div style={{ padding: '28px', background: '#f7fafc', minHeight: '100vh' }}>
      ...
      <div style={{ marginBottom: 28 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 6 }}>
          ...
          <div>
            <h1 style={{ margin: 0, fontSize: 22, fontWeight: 800, color: '#1a202c' }}>Transport Management</h1>
            <div style={{ fontSize: 13, color: '#718096' }}>Overview of fleet, routes, and student assignments</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 20, marginBottom: 20 }}>
        ...
        <div style={{ background: '#fff', borderRadius: 16, padding: 24, boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
          <h3 style={{ margin: '0 0 16px', fontSize: 15, fontWeight: 700, color: '#2d3748' }}>Fleet Utilization</h3>
          <div style={{ marginBottom: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
              <span style={{ fontSize: 13, color: '#4a5568' }}>Overall Capacity Used</span>
              <span style={{ fontSize: 13, fontWeight: 700, color: '#2d3748' }}>{totalStudents}/{totalCapacity} ({utilPct}%)</span>
            </div>
          ...
          {buses.slice(0, 4).map(bus => (
            <div key={bus.id} style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 12, padding: '10px 0', borderTop: '1px solid #f0f4f8' }}>
              <span className="material-icons" style={{ fontSize: 18, color: '#3182ce' }}>directions_bus</span>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: '#2d3748' }}>{bus.busNo}</div>
                <div style={{ fontSize: 11, color: '#a0aec0' }}>{bus.route || 'No route assigned'}</div>
              </div>
              <div style={{ fontSize: 12, color: '#4a5568', marginRight: 8 }}>{bus.currentStudents}/{bus.capacity}</div>
              <div style={{ height: 6, width: 80, background: '#e2e8f0', borderRadius: 6, overflow: 'hidden' }}>
        ...
        {/* Recent Activity */}
        <div style={{ background: '#fff', borderRadius: 16, padding: 24, boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
          <h3 style={{ margin: '0 0 16px', fontSize: 15, fontWeight: 700, color: '#2d3748' }}>Recent Activity</h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {recentActivities.map((a, i) => (
              <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 12, color: '#4a5568', lineHeight: 1.4 }}>{a.text}</div>
      ...
      <div style={{ background: '#fff', borderRadius: 16, padding: 24, boxShadow: '0 2px 12px rgba(0,0,0,0.06)', marginBottom: 20 }}>
        <h3 style={{ margin: '0 0 16px', fontSize: 15, fontWeight: 700, color: '#2d3748' }}>Quick Actions</h3>
      ...
      {/* Routes Overview */}
      <div style={{ background: '#fff', borderRadius: 16, padding: 24, boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
        <h3 style={{ margin: '0 0 16px', fontSize: 15, fontWeight: 700, color: '#2d3748' }}>Routes Overview</h3>
        {routes.length === 0 ? (
              ...
              <thead>
                <tr style={{ background: '#f7fafc' }}>
                  {['Route', 'Bus', 'Driver', 'Stops', 'Distance', 'Pickup', 'Status'].map(h => (
                    <th key={h} style={{ padding: '10px 14px', textAlign: 'left', fontWeight: 700, color: '#718096', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {routes.map(r => (
                  <tr key={r.id} style={{ borderTop: '1px solid #f0f4f8' }}>
                    <td style={{ padding: '10px 14px', fontWeight: 600, color: '#2d3748' }}>{r.name}</td>
                    <td style={{ padding: '10px 14px', color: '#4a5568' }}>{r.busNo || '—'}</td>
                    <td style={{ padding: '10px 14px', color: '#4a5568' }}>{r.driverName || '—'}</td>
                    <td style={{ padding: '10px 14px', color: '#4a5568' }}>{r.stops || 0}</td>
                    <td style={{ padding: '10px 14px', color: '#4a5568' }}>{r.distance || '—'}</td>
                    <td style={{ padding: '10px 14px', color: '#4a5568' }}>{r.pickupTime || '—'}</td>
