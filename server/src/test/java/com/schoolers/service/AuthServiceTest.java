...
class AuthServiceTest {
    ...
    class LoginTests {
        ...
        void fiveWrongPasswords_locksAccount() {
            ...
            // Set updatedAt within the 30-min window so the sliding-window code does NOT
            // reset the counter before incrementing (updatedAt acts as the last-attempt proxy).
            user.setUpdatedAt(LocalDateTime.now(ZoneOffset.UTC).minusMinutes(5));
            when(userRepository.findByEmailIgnoreCase("admin@school.com")).thenReturn(Optional.of(user));
            when(schoolRepository.findBySchoolId(5)).thenReturn(Optional.of(activeSchool()));
