...
public interface WhatsAppQueueRepository extends JpaRepository<WhatsAppQueueItem, Long> {
     ...
     */
    @Query(value = "SELECT id FROM whatsapp_queue WHERE status = 'PENDING' " +
    ...
    @Modifying
    @Query("UPDATE WhatsAppQueueItem q SET q.status = com.schoolers.model.sms.QueueStatus.PROCESSING, q.updatedAt = CURRENT_TIMESTAMP WHERE q.id IN :ids")
    int markProcessing(@Param("ids") List<Long> ids);

    long countByCampaignIdAndStatus(Long campaignId, QueueStatus status);

    List<WhatsAppQueueItem> findByCampaignId(Long campaignId);

    @Modifying
    @Query("UPDATE WhatsAppQueueItem q SET q.status = com.schoolers.model.sms.QueueStatus.CANCELLED, q.updatedAt = CURRENT_TIMESTAMP " +
    ...
    @Query("SELECT CASE WHEN COUNT(q) = 0 THEN true ELSE false END FROM WhatsAppQueueItem q " +
