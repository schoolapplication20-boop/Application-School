...
public class AdminService {
    ...
    public ApiResponse<FeeInstallment> collectInstallmentFee(Long installmentId, Map<String, Object> body, Long schoolId) {
        ...
        // Each installment is fully self-contained: its due amount is its own base amount
        // minus whatever has already been paid toward it — never inflated by another term's
        // shortfall, and never reduced by paying it forward either.
        ...
        BigDecimal effectiveDue = base.subtract(prevPaid);
        ...
        BigDecimal shortage = base.subtract(totalPaidForTerm);
        boolean fullyCovered = shortage.compareTo(BigDecimal.ZERO) <= 0;

        // A shortfall stays on THIS term (status PARTIAL) — it is never added to another
        // installment's amount. The admin can come back to this same term later, while it's
        // still PARTIAL, to collect the remainder.
        installment.setStatus(fullyCovered ? FeeInstallment.Status.PAID : FeeInstallment.Status.PARTIAL);
        ...
        String msg = fullyCovered
            ...
            : "Partial payment recorded for " + installment.getTermName() + ". ₹"
              + shortage.setScale(0, java.math.RoundingMode.HALF_UP) + " still due on this term.";
