...
public class SalaryService {
    ...
    public ApiResponse<Salary> create(Map<String, Object> body, Long schoolId) {
        ...
        BigDecimal basic = decimal(body, "basic", salary.getBasic() != null ? salary.getBasic() : BigDecimal.ZERO);
        if (basic.compareTo(BigDecimal.ZERO) < 0) return ApiResponse.error("Basic salary cannot be negative");
        salary.setBasic(basic);
        salary.setHra(decimal(body, "hra", salary.getHra() != null ? salary.getHra() : BigDecimal.ZERO).max(BigDecimal.ZERO));
        salary.setDa(decimal(body, "da", salary.getDa() != null ? salary.getDa() : BigDecimal.ZERO).max(BigDecimal.ZERO));
        salary.setMedical(decimal(body, "medical", salary.getMedical() != null ? salary.getMedical() : BigDecimal.ZERO).max(BigDecimal.ZERO));
        salary.setPf(decimal(body, "pf", salary.getPf() != null ? salary.getPf() : BigDecimal.ZERO).max(BigDecimal.ZERO));
        salary.setTax(decimal(body, "tax", salary.getTax() != null ? salary.getTax() : BigDecimal.ZERO).max(BigDecimal.ZERO));
